...
class mtl():
    ...
    def assign(self):
        ...
        def make_urls(url):
            ...
            print(title)
            info_list = result.xpath("/html/body/div[2]/div[2]/p/text()")  # 获取含有图片数量的列表(扩大了范围）
            for item in info_list:
                print(item)
            s = ''.join(info_list)  # 合成为一个字符串
            print(s)
            num = search(r'(?<=图片数量：).*(?=张)', s , M).group().strip()  # 提取图片数量
        ...
        try:
            ...
            self.download()
            print("按地址序号下载功能added by 吾爱破解 河瞬\n原作者@吾爱破解 lihaisanhui")
            address = input("按Enter键退出...\n")
        except:
            print("本模块出现错误，请检查地址编号。如确认无误联系作者并提交需爬取的页面编号")
            c = input("是否反馈问题？（回车键退出，输入任意字符回车进入反馈页面）")
            if c != "":
                webbrowser_open("https://github.com/heshunme/Python_Work/issues")
    ...
    def run(self):
        while True:
            choose = input('请选择操作模式[1=搜索，2=标签, 3=直接输入地址序号，4=查看帮助页]： ')
            if choose == '1':
                self.search()
                break
            elif choose == '2':
...
if __name__ == "__main__":
    system('title MeiTuLuSpider[V2.2.1] @吾爱破解 lihaisanhui')
    print('欢迎使用美图录Spider[V2.2.1-2020.03.17]非官方修改版！\n前往数据源：https://www.meitulu.com 下载更多精彩图片！\n')
    colorama.init(True)
